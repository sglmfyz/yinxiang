...
/* Private functions ---------------------------------------------------------*/
static int EE_Format(void);
static int EE_VerifyPageFullWriteVariable(uint16_t VirtAddress, uint16_t Data);
static int EE_PageTransfer(uint16_t VirtAddress, uint16_t Data);
static int EE_FindValidPage(uint8_t Operation);

#define FLASH_COMPLETE HAL_OK

static int FLASH_ErasePage(uint32_t page_addr)
{
  #ERROR 1
    ...
    init.PageAddress = page_addr;
    init.NbPages = 1;


    status = HAL_FLASHEx_Erase(&init, &error);
    if (status != 0) {
        syslog_error("Page Erase error, %d\n", page_addr);
    }
    return status;
}
#define FLASH_ProgramHalfWord(addr, data) HAL_FLASH_Program(FLASH_TYPEPROGRAM_HALFWORD, addr, data)
...
int EE_Init(void)
{
  ...
  syslog_debug("PageStatus0:%d, PageStatus1:%d\n", PageStatus0, PageStatus1);
