...
typedef enum{
  ...
  ACK_ERROR      = 0x101, // no acknowledgment error
  CHECKSUM_ERROR = 0x102, // checksum mismatch error
  TIMEOUT_ERROR  = 0x104, // timeout error
  PARM_ERROR     = 0x180, // parameter out of range error
}etSht3xError;
...
struct SHT3X {
    //t (*Read_SerialNum)(SHT3X_T *this, uint8_t addr, uint32_t *Serial_Num);
    void (*Set_I2C_Addr)(SHT3X_T *this, I2C_T *i2c, uint8_t addr);
    int (*Start_Periodic_Measurment)(SHT3X_T *this, 
                                      etRepeatability repeatability,
                                      etFrequency frequency);
    int (*Read_Measurement_Buffer)(SHT3X_T *this, 
